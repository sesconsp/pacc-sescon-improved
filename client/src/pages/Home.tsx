                ...
                <motion.div key="aba1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="bg-white rounded-2xl shadow-xl border p-8" style={{ borderColor: SESCON_LIGHT_BLUE }}>
                  ...
                  <div className="space-y-6 w-full">
                    <div className="space-y-2">
                      <label className="text-sm font-bold flex items-center gap-1" style={{ color: SESCON_DARK_BLUE }}>CNPJ do Escritório <span className="text-red-500">*</span></label>
                      <div className="relative">
                        <Input type="text" placeholder="00.000.000/0000-00" value={cnpjEscritorio} onChange={(e) => setCnpjEscritorio(formatarCNPJ(e.target.value))} onBlur={() => buscarCNPJ(cnpjEscritorio)} className="rounded-lg border-2 px-4 py-3 text-lg w-full" style={{ borderColor: SESCON_BLUE }} maxLength={18} />
                    ...
                    <div className="space-y-2">
                      ...
                      <Input type="text" placeholder="Será preenchido automaticamente" value={razaoSocialEscritorio} readOnly className="rounded-lg border-2 px-4 py-3 bg-gray-50 w-full cursor-not-allowed" style={{ borderColor: SESCON_BLUE }} />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input type="email" placeholder="contato@empresa.com.br" value={emailEscritorio} onChange={(e) => setEmailEscritorio(e.target.value)} disabled={!cnpjEscritorioValido} className={`rounded-lg border-2 px-4 py-3 w-full ${!cnpjEscritorioValido ? 'bg-gray-100 cursor-not-allowed' : ''}`} style={{ borderColor: SESCON_BLUE }} />
                    </div>
                    <Button onClick={() => setAbaSelecionada(2)} disabled={!podeAvancarAba1} className={`w-full rounded-lg font-bold py-4 text-white text-lg mt-4 shadow-md ${!podeAvancarAba1 ? 'opacity-50 cursor-not-allowed' : ''}`} style={{ background: SESCON_BLUE }}>Próximo</Button>
                  </div>
                      ...
                      <div className="space-y-3">
                        {faqs.map((faq, idx) => (
                          <div key={idx} className="border rounded-lg overflow-hidden" style={{ borderColor: SESCON_LIGHT_BLUE }}>
                            <button onClick={() => setFaqAberto(faqAberto === idx ? null : idx)} className="w-full px-6 py-4 text-left flex justify-between items-center bg-white hover:bg-gray-50 transition-colors">
                            ...
                            <AnimatePresence>
                              {faqAberto === idx && (
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-white px-6 pb-4">
                ...
                <motion.div key="aba2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="bg-white rounded-2xl shadow-xl border p-8" style={{ borderColor: SESCON_LIGHT_BLUE }}>
                  ...
                  <div className="space-y-8">
                    <div className="p-6 rounded-xl border-2" style={{ borderColor: SESCON_LIGHT_BLUE, background: "#fcfdfe" }}>
                      ...
                      <div className="flex gap-8">
                        <label className="flex items-center gap-3 cursor-pointer group">
                          <input type="radio" name="atividade" value="contabilidade" checked={atividadePrincipal === "contabilidade"} onChange={(e) => setAtividadePrincipal(e.target.value)} className="w-5 h-5" style={{ accentColor: SESCON_BLUE }} />
                          <span className="text-sm font-semibold">Contabilidade</span>
                        </label>
                        <label className="flex items-center gap-3 cursor-pointer group">
                          <input type="radio" name="atividade" value="outros" checked={atividadePrincipal === "outros"} onChange={(e) => setAtividadePrincipal(e.target.value)} className="w-5 h-5" style={{ accentColor: SESCON_BLUE }} />
                    ...
                    <div className="grid grid-cols-2 gap-8">
                      <div className="space-y-4">
                        <h3 className="text-lg font-bold flex items-center gap-2" style={{ color: SESCON_DARK_BLUE }}><Upload className="w-5 h-5" />Importar Planilha (.xlsx, .csv)</h3>
                        <div className="border-2 border-dashed rounded-xl p-8 text-center hover:bg-blue-50 transition-all cursor-pointer relative group" style={{ borderColor: SESCON_BLUE }}>
                          ...
                          <div className="space-y-2">
                            <div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center mx-auto"><FileDown className="w-6 h-6" style={{ color: SESCON_BLUE }} /></div>
                            <p className="text-sm font-bold" style={{ color: SESCON_BLUE }}>Clique ou arraste sua planilha aqui</p>
                          </div>
                        </div>
                        <Button onClick={baixarModelo} variant="ghost" size="sm" className="w-full text-xs font-bold hover:bg-blue-50" style={{ color: SESCON_BLUE }}><DownloadIcon className="w-3 h-3 mr-1" />Baixar Planilha Modelo</Button>
                      </div>

                      <div className="space-y-4">
                        <h3 className="text-lg font-bold flex items-center gap-2" style={{ color: SESCON_DARK_BLUE }}><Plus className="w-5 h-5" />Adicionar Cliente</h3>
                        <div className="space-y-3">
                          <Input placeholder="CNPJ do Cliente" value={novoCliente.cnpj} onChange={(e) => setNovoCliente({ ...novoCliente, cnpj: formatarCNPJ(e.target.value) })} onBlur={() => buscarCNPJ(novoCliente.cnpj || "", true)} className="rounded-lg border-2" style={{ borderColor: SESCON_BLUE }} />
                          <Input placeholder="Razão Social" value={novoCliente.razaoSocial} onChange={(e) => setNovoCliente({ ...novoCliente, razaoSocial: e.target.value })} className="rounded-lg border-2" style={{ borderColor: SESCON_BLUE }} />
                          <div className="grid grid-cols-2 gap-2">
                            <div className="relative"><DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Faturamento Anual" value={novoCliente.faturamento} onChange={(e) => setNovoCliente({ ...novoCliente, faturamento: formatarMoeda(e.target.value) })} className="rounded-lg border-2 pl-9" style={{ borderColor: SESCON_BLUE }} /></div>
                            <div className="relative"><Users className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Nº Funcionários" value={novoCliente.funcionarios} onChange={(e) => setNovoCliente({ ...novoCliente, funcionarios: e.target.value.replace(/\D/g, "") })} className="rounded-lg border-2 pl-9" style={{ borderColor: SESCON_BLUE }} /></div>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <div className="relative"><Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="E-mail Empresa" value={novoCliente.emailEmpresa} onChange={(e) => setNovoCliente({ ...novoCliente, emailEmpresa: e.target.value })} className="rounded-lg border-2 pl-9" style={{ borderColor: SESCON_BLUE }} /></div>
                            <div className="relative"><Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Telefone Empresa" value={novoCliente.telefoneEmpresa} onChange={(e) => setNovoCliente({ ...novoCliente, telefoneEmpresa: formatarTelefone(e.target.value) })} className="rounded-lg border-2 pl-9" style={{ borderColor: SESCON_BLUE }} /></div>
                          </div>
                          <Button onClick={adicionarCliente} className="w-full rounded-lg font-semibold py-2 text-white" style={{ background: SESCON_BLUE }}><Plus className="w-4 h-4 mr-2" />Adicionar Cliente</Button>
                        </div>
                      </div>
                    </div>

                    {/* CAIXA DE VISUALIZAÇÃO DE CLIENTES (FIXA NA TELA) */}
                    <div className="p-6 rounded-xl border-2 bg-white shadow-sm" style={{ borderColor: SESCON_LIGHT_BLUE }}>
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold" style={{ color: SESCON_DARK_BLUE }}>Clientes Adicionados ({clientes.length})</h3>
                        <div className="relative w-64">
                          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                          <Input placeholder="Pesquisar na lista..." value={busca} onChange={(e) => setBusca(e.target.value)} className="pl-10 h-9 text-sm" />
                        </div>
                      </div>
                      <div className="max-h-64 overflow-y-auto space-y-2 pr-2">
                        {clientesFiltrados.length === 0 ? (
                          <div className="text-center py-8 text-gray-400 italic text-sm">Nenhum cliente adicionado ainda.</div>
                        ) : (
                          clientesFiltrados.map((cliente, idx) => (
                            <div key={cliente.id} className="p-3 rounded-lg border flex justify-between items-center bg-gray-50 hover:bg-blue-50 transition-colors" style={{ borderColor: SESCON_LIGHT_BLUE }}>
                              <div className="flex-1 min-w-0">
                                <p className="font-bold text-xs truncate" style={{ color: SESCON_DARK_BLUE }}>{idx + 1}. {cliente.razaoSocial}</p>
                                <p className="text-[10px] text-gray-500">{cliente.cnpj} | {cliente.emailEmpresa || "Sem e-mail"}</p>
                              </div>
                              <Button onClick={() => removerCliente(cliente.id)} variant="ghost" size="sm" className="text-red-500 hover:bg-red-100 h-8 w-8 p-0"><Trash2 className="w-4 h-4" /></Button>
                            </div>
                          ))
                        )}
                      </div>
                    ...
                    <div className="flex flex-col gap-3 pt-4">
                      {isLoading && (
                        <div className="space-y-2 mb-4">
                          <div className="flex justify-between text-xs font-bold" style={{ color: SESCON_BLUE }}><span>Enviando dados para o SESCON-SP...</span><span>{progressoEnvio}%</span></div>
                          <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${progressoEnvio}%` }} className="h-full bg-green-500" /></div>
                        </div>
                      )}
                      ...
                      <Button onClick={enviarDados} disabled={isLoading || clientes.length === 0} className="flex-1 rounded-lg font-bold py-3 text-white text-lg hover:bg-green-700 transition-colors" style={{ background: "#4CAF50" }}>
      ...
      <AlertDialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <AlertDialogContent className="bg-white rounded-3xl border-none shadow-2xl p-8 max-w-md mx-auto">
          <AlertDialogHeader className="items-center text-center space-y-4">
            <div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-2"><CheckCircle className="w-10 h-10" /></div>
            <AlertDialogTitle className="text-2xl font-bold text-slate-800">Sucesso!</AlertDialogTitle>
            <AlertDialogDescription className="text-slate-600 text-lg">Sua atualização cadastral foi recebida com sucesso. Um e-mail de confirmação foi enviado para <strong>{emailEscritorio}</strong>.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8"><AlertDialogAction onClick={resetForm} className="w-full h-14 bg-[#003b61] hover:bg-[#00568c] text-lg font-bold rounded-xl shadow-lg shadow-blue-900/20">Concluir e Voltar ao Início</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <footer className="pt-8 pb-6 px-8" style={{ background: "#003366" }}>
        <div className="max-w-6xl mx-auto text-white">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-6 mb-6 border-b border-white border-opacity-30">
            ...
            <div className="hidden md:flex justify-end"><img src="/pacc-sescon-improved/logo-sescon-branco.png" alt="SESCON-SP" className="h-20 w-auto" /></div>
          </div>
